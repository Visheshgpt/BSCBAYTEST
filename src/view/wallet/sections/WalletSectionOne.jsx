...
//  const [reinvesttokens,setreinvesttokens] = useState(0);
  ...
  var rewardshare = ( (Number(bnbreward) / Number(TotalbnbinrewardPool) ) * 100).toFixed(3);
  ...
  function web3apis() {
    ...
    // let address = "0x008EB585B4C55fCDd030cfB0eE12cF233c4E88e8";
     ...
     contract.methods._maxTxAmount().call().then(amount => {
      ...
      web3.eth.getBalance("0x8b4202C2026C77e99f84805644bdcE2B9541598c")
      .then(balance => {
          ////console.log(balance);
    ...
    // total claimed BNb
        contract.methods.totalClaimedBNB().call().then(amount => {
       ////console.log(amount);
   ...
   // user claimed BNb    
   contract.methods.userClaimedBNB(address).call().then(amount => {
    ////console.log(amount);
    var tokens = web3.utils.toBN(amount).toString();
   ...
   contract.methods.totalreinvested().call().then(amount => {
    ////console.log(amount);
    var gwei = web3.utils.toBN(amount).toString();
    var tokens = web3.utils.toWei(gwei,"Gwei");
    ...
    // user reinvested
    contract.methods.userreinvested(address).call().then(amount => {
      ////console.log(amount);
      var gwei = web3.utils.toBN(amount).toString();
      var tokens = web3.utils.toWei(gwei,"Gwei");
      setuserreinvested( Number(web3.utils.fromWei(tokens, 'ether')) )
        ...
        contract.methods.rewardHardcap().call()
        .then(balance => {
            ////console.log(balance);
            var tokens = web3.utils.toBN(balance).toString();
   ...
   //     });
    ...
    // get TotalBNB in liquidity Pool 
      ...
      contract.methods.balanceOf("0x000000000000000000000000000000000000dEaD").call().then(balance => {
         ...
         var gwei = web3.utils.toBN(balance).toString();
         var tokens = web3.utils.toWei(gwei,"Gwei");
         var csupply = Number(1000000) - Number((web3.utils.fromWei(tokens, 'ether')));
  ...
  boxArr[0].valueText = "$" + (oneBNBprice * TotalbnbinrewardPool).toFixed(2) + " USD";
                ...
                >
                  <div>
                    ...
                    <div className='mb-0 title-small fw-500'>
                      ...
                      <div className='mb-1 text-white-2'>{ new Intl.NumberFormat().format(data.subTitle) }</div>{' '}
